...
public:
    ArcticEmbedTiny(const std::string& model_path, const std::string& vocab_path = "") : unk_token_id_(100), cls_token_id_(101), sep_token_id_(102), pad_token_id_(0), max_length_(512) {
        ...
        // Optimize for CPU with more threads
        session_options.SetIntraOpNumThreads(4);  // Increased from 2 to 4
        session_options.SetInterOpNumThreads(4);  // Increased from 2 to 4
...
#ifdef USE_COREML
#ifdef COREML_PROVIDER_AVAILABLE
        // Enable CoreML acceleration with Apple Neural Engine targeting
        OrtStatus* status = OrtSessionOptionsAppendExecutionProvider_CoreML(session_options, COREML_FLAG_USE_ANE);  // Target ANE
        if (status != nullptr) {
            const char* error_message;
            OrtGetErrorMessage(status, &error_message);
            std::cerr << "Warning: Failed to enable CoreML execution provider with ANE: " << (error_message ? error_message : "Unknown error") << std::endl;
            OrtReleaseStatus(status);
            
            // Fallback to CPU if ANE fails
            std::cout << "Falling back to CPU execution." << std::endl;
        } else {
            std::cout << "CoreML execution provider with ANE enabled successfully." << std::endl;
        }
#else
    ...
    // Warmup the model to improve subsequent inference performance
    void warmup_model() {
        std::cout << "Performing model warmup..." << std::endl;
        
        // Create a simple dummy input for warmup
        std::string dummy_text = "warmup text for initialization";
        std::vector<int64_t> dummy_input_ids = tokenize_text(dummy_text);
        
        // Pad to a reasonable size for warmup
        if (dummy_input_ids.size() < 10) {
            dummy_input_ids.resize(10, pad_token_id_);
        }
        
        std::vector<int64_t> dummy_attention_mask(dummy_input_ids.size(), 1);
        std::vector<int64_t> dummy_token_type_ids(dummy_input_ids.size(), 0);
        ...
        if (!input_node_dims_.empty() && !input_node_dims_[0].empty()) {
            ...
            if (input_shape.size() >= 2 && (input_shape[1] == -1 || input_shape[1] <= 0)) {
                input_shape[1] = static_cast<int64_t>(dummy_input_ids.size());
            } else if (input_shape.size() == 1) {
                input_shape.push_back(static_cast<int64_t>(dummy_input_ids.size()));
            }
        } else {
            input_shape = {1, static_cast<int64_t>(dummy_input_ids.size())};
        }
        ...
        // Create input tensors for warmup
        ...
        if (num_inputs >= 1) {
            ...
            if (!input_node_dims_[0].empty()) {
                actual_input_shape = fix_tensor_shape(input_node_dims_[0], dummy_input_ids.size());
            }

            input_tensors.push_back(Ort::Value::CreateTensor<int64_t>(
                memory_info,
                dummy_input_ids.data(),
                dummy_input_ids.size(),
        ...
        if (num_inputs >= 2) {
            ...
            if (num_inputs >= 2 && !input_node_dims_[1].empty()) {
                mask_shape = fix_tensor_shape(input_node_dims_[1], dummy_attention_mask.size());
            } else {
                mask_shape = fix_tensor_shape(input_shape, dummy_attention_mask.size());
            }

            input_tensors.push_back(Ort::Value::CreateTensor<int64_t>(
                memory_info,
                dummy_attention_mask.data(),
                dummy_attention_mask.size(),
                mask_shape.data(),
                mask_shape.size()
            ));
        }
        ...
        if (num_inputs >= 3) {
            ...
            if (num_inputs >= 3 && !input_node_dims_[2].empty()) {
                token_type_shape = fix_tensor_shape(input_node_dims_[2], dummy_token_type_ids.size());
            } else {
                token_type_shape = fix_tensor_shape(input_shape, dummy_token_type_ids.size());
            }

            input_tensors.push_back(Ort::Value::CreateTensor<int64_t>(
                memory_info,
                dummy_token_type_ids.data(),
                dummy_token_type_ids.size(),
        ...
        try {
            ...
            std::cout << "Model warmup completed successfully." << std::endl;
    ...
    std::vector<float> embed(const std::string& text) {
        ...
        // Resize pre-allocated buffers to accommodate the input
        input_ids_buffer_.resize(input_ids.size());
        attention_mask_buffer_.resize(attention_mask.size());
        token_type_ids_buffer_.resize(token_type_ids.size());
        
        // Copy data to pre-allocated buffers
        std::copy(input_ids.begin(), input_ids.end(), input_ids_buffer_.begin());
        std::copy(attention_mask.begin(), attention_mask.end(), attention_mask_buffer_.begin());
        std::copy(token_type_ids.begin(), token_type_ids.end(), token_type_ids_buffer_.begin());
        ...
        try {
            ...
            // Resize pooled_result_buffer_ to the appropriate size
            pooled_result_buffer_.resize(hidden_size, 0.0f);
            
            // Apply mean pooling with attention mask using Accelerate framework
            ...
            // Use Accelerate framework for vectorized mean pooling
            if (valid_tokens_count > 0) {
                // Initialize pooled_result_buffer_ to zero
                vDSP_vclr(pooled_result_buffer_.data(), 1, hidden_size);
                ...
                for (int s = 0; s < actual_seq_len; ++s) {
                    if (attention_mask[s] == 1) {  // Only consider unmasked tokens
                        // Add the current token's embedding to the accumulated result
                        cblas_saxpy(hidden_size, 1.0f, 
                                   &float_array_data[s * hidden_size], 1, 
                                   pooled_result_buffer_.data(), 1);
                    }
                }

                // Divide by valid_tokens_count to get the mean
                float divisor = 1.0f / valid_tokens_count;
                cblas_sscal(hidden_size, divisor, pooled_result_buffer_.data(), 1);
            } else {
                // Handle edge case where no tokens are valid
                std::fill(pooled_result_buffer_.begin(), pooled_result_buffer_.end(), 0.0f);
            }

            // Ensure output buffer is always exactly 384 dimensions
            result_buffer_.resize(384, 0.0f);

            // Copy values from pooled_result_buffer_ to result_buffer_ of fixed size 384
            ...
            // Normalize the embedding (L2 normalization) using Accelerate framework
            normalize_embedding_accelerate(result_buffer_);

#ifndef DISABLE_JSON_OUTPUT
            // Output only the vector as JSON for easy parsing (optional)
            std::cout << "[";
            ...
            // Return a copy of the result buffer
            return std::vector<float>(result_buffer_.begin(), result_buffer_.begin() + 384);
...
private:
    ...
    // Normalize the embedding using Accelerate framework for better performance
    void normalize_embedding_accelerate(std::vector<float>& embedding) {
        // Calculate L2 norm using Accelerate framework
        float norm = cblas_snrm2(embedding.size(), embedding.data(), 1);
        
        if (norm > 0.0f) {
            // Normalize using Accelerate framework
            float inv_norm = 1.0f / norm;
            cblas_sscal(embedding.size(), inv_norm, embedding.data(), 1);
        }
    }
