...
public:
    ArcticEmbedTiny(const std::string& model_path, const std::string& vocab_path = "") : unk_token_id_(100), cls_token_id_(101), sep_token_id_(102), pad_token_id_(0), max_length_(512) {
...
#ifdef USE_COREML
#ifdef COREML_PROVIDER_AVAILABLE
        // Enable CoreML acceleration with default settings (no ANE flag)
        OrtStatus* status = OrtSessionOptionsAppendExecutionProvider_CoreML(session_options, 0);  // No special flags
        if (status != nullptr) {
        ...
        // Removed warmup_model() call - it adds overhead without benefit
    }
